...
func verifyRouterOSConsole(console *serialConsole, plan model.NetworkPlan) error {
	...
	checks := []struct {
		...
		{`:put ("CHRTEST-" . "GW4SCOPE:" . [/ip/route/get [find where comment="chr-install" and dst-address="0.0.0.0/0"] target-scope])`, "CHRTEST-GW4SCOPE:", "11"},
		{`:put ("CHRTEST-" . "GW4ACTIVE:" . [/routing/route/print count-only where comment="chr-install" and dst-address="0.0.0.0/0" and active])`, "CHRTEST-GW4ACTIVE:", "1"},
		{`:put ("CHRTEST-" . "GW4IMMEDIATE:" . [/routing/route/get [find where comment="chr-install" and dst-address="0.0.0.0/0" and active] immediate-gw])`, "CHRTEST-GW4IMMEDIATE:", plan.IPv4.Gateway + "%" + interfaceName},
